...
function tunnelRawDataHandler(clientSocket, data) {
    ...
    if (dataParts.length > 1) {
        if (dataParts[0] === "beat" && dataParts.length == 5) {
            ...
            const deviceObject = activeIoTDevices.createActiveDevice(
                user,
                hostName,
                macAddress,
                deviceType,
            ...
            if (
                !activeIoTDevices.checkDevice(
                    activeIoTDevices.getKey(user, macAddress)
                )
            ) {
                ...
                database.User.findOne({ uuid: user })
                    .then((foundUser) => {
                        if (!foundUser) {
                            ...
                            logger.warn(
                                `User is not in the DB, message is dropped. | User: ${user}`
                            );
                            return;
                        }
            ...
            //Device is in ram list, => has to exist in DB
            else {
                activeIoTDevices.addOrUpdateDevice(deviceObject);
                //SAVE DB ONLY IoT type devices
                if (deviceType == 0) saveHearthbeatDB(deviceObject);
            ...
            const destinationDevice = activeConnections.getOtherHalf(
                uuid,
                sourceDevice
            );
            ...
            activeConnections.addConnection(
                activeIoTDevices.getDevice(user, dev1MAC),
                activeIoTDevices.getDevice(user, dev2MAC)
            );
        } else {
...
function saveHearthbeatDB(deviceObject) {
    ...
    database.Device.findOneAndUpdate(
        { macAddress: deviceObject.macAddress },
        deviceDB,
        { upsert: true, new: true }
    )
        .then((device) => {
            logger.debug("Device updated or created:", device);
        })
        .catch((err) => {
